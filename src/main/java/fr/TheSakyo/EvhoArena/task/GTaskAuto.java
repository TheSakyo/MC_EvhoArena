				...
				for(int i = 0; i < main.manager.getPlayers().size(); i++) {
					...
					if(location != null) {
						...
						while(!hasNoAir) {
							...
							location.setY(centerZone.getY()); // Définit à la localisation récupéré aléatoirement la valeur Y de la localisation du centre de la zone du mini-jeux
							List<Location> aroundLoc = CustomMethod.getNearbyLocations(location, 3, false); // On récupère une liste de localisations autour d'une autre localisation précisée
